...
var cross = (function(createMove, edgeMove, createPrun, setNPerm, getNPerm, Cnk, getPruning) {
    ...
    function solve_cross(scramble, fdiv) {
        init();
        fdiv.empty();
        var moves = curScramble = parseScramble(scramble, "FRUBLD");// = scramble.split(/\s+/);
    ...
    function execFunc(div) {
            var scramble = $('#scramble').val().replace(/\n/g,'');
            solve_cross(scramble, div);
