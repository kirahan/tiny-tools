(function(circle) {
	...
	function initCornerMoveTable() {
		...
		for (var i=0; i<40320; i++) {
			...
			circle(g, 0, 1, 2, 3);
			temp = cmv[0][i] = mathlib.get8Perm(g);//U
			circle(g, 4, 5, 6, 7);
			temp = cmv[1][temp] = mathlib.get8Perm(g);//D
			circle(g, 2, 5)(g, 3, 6);
			temp = cmv[2][temp] = mathlib.get8Perm(g);//R
			circle(g, 0, 5)(g, 3, 4);
			cmv[3][temp] = mathlib.get8Perm(g);//F
		}
	}
