...
/*	if (type && type.substr(0, 5) == 'race_') {
		contest.getScramble(type, doScrambleIt);
	} else*/
	scramble = "";

	if (type in scramblers) {
		scramble = scramblers[type](type, len);
		return;
	}
...
function oldminxscramble(){
	...
	function procSignal(signal, value) {
		if (signal == 'time') {
			...
			if (value[0] == 'scrSize') {
				...
				sdiv.css('max-height', value[1] ? '150px' : '');
			}
	...
	var scrambleGenerator = (function() {
		...
		var scrNum = $('<input type="text" maxlength="3">').width(30).val(5);
	...
	$(function(){
		...
		kernel.regListener('scramble', 'property', procSignal, /^scr(?:Size|Mono|Type|Lim)$/);
		...
		scrLen.change(genScramble).width(30);
		...
		title.append(select, select2, ' ' + SCRAMBLE_LENGTH + ': ', scrLen, ' ', last, '/', next, SCRAMBLE_SCRAMBLE);
		div.append(title, sdiv).appendTo('body');
		kernel.addWindow('scramble', BUTTON_SCRAMBLE, div, false, true);
